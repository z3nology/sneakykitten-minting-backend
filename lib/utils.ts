...
import { BACKEND_ADDRESS, COLLECTION_SYMBOL, ROYALTY_ADDR, backendKp, backendWallet, connection, COLLECTION_NAME, COLLECTION_URI } from './config';
...
const mintMasterEdition = async (
    ...
    for (let i = 0; i < maxTryTime; i++) {
        let confirmed = null;
        try {
            confirmed = await metaplex.rpc().sendAndConfirmTransaction(transactionBuilder, { commitment: 'confirmed', maxRetries: 10 });
        } catch (e) {
            console.log(e);
        }
        if (confirmed != null && confirmed.confirmResponse.value.err == null)
            break;
    ...
    for (let i = 0; i < maxTryTime; i++) {
        try {
            nft = await metaplex.nfts().findByMint({ mintAddress }, { commitment: 'confirmed' });
        } catch (e) {
